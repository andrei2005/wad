...
client.on('message', message => {
    if (message.author.id == '399658695056949249') {
...
client.on('message', msg => {
    if (msg.content === 'e!invite') {
      msg.channel.send('Invite **ll Snoops Bot ll** https://discordapp.com/api/oauth2/authorize?client_id=463774464971112469&permissions=8&scope=bot  :tada:');
    }
  });
...
client.on('guildMemberAdd', member => {
    var joinrole = member.guild.roles.find('name', 'membru');
    ...
    let memberavatar  = member.user.avatarURL
        ...
        .addField('? | Welcome', `**Bine ai venit pe server-ul [E.W] Elite Warriors™. Speram sa te distrezi alaturi de noi. Si nu uita sa citesti :beginner:??ules:beginner:-ul**`)
        .setTimestamp()
...
client.on('guildMemberRemove', member => {
    let channel = member.guild.channels.find('name', '?????elcome-??oodbye???');
    let memberavatar = member.user.avatarURL
        if (!channel) return;
        let embed = new Discord.RichEmbed()
        .setColor('#ff0000')
...
client.login(process.env.TOKEN);const Discord = require('discord.js');
const client = new Discord.Client();
const prefix = "e!"
...
client.login(process.env.TOKEN);        channel.sendEmbed(embed);
});
